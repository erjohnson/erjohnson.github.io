...
object inconsistencies using [read repair](##) alone:
...
The default directory for AAE data is `./data/anti_entropy` but this can be changed. See the [Data Directory](#data-directory) section below.

Remember you will need to [restart the node](##) for any configuration-related changes to take effect.
...
[vnode](##) in your cluster in these three categories:
...
AAE repair operations when [vnode](##) mailboxes reach the length
...
AAE-related background tasks, analogous to [open files limit](##) settings used in operating systems. The default is `20`.

## AAE and Riak Search

Riak's AAE subsystem works to repair object inconsistencies both with
for normal key/value objects as well as data related to [Riak Search](##). In particular, AAE acts on indexes stored in
[Solr](##), the search platform that drives
